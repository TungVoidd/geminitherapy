...
function createTrayIconFallback() {
  ...
  console.warn('geminitherapy: icon/Template.png missing or invalid');
...
async function getTrayIcon() {
  ...
  if (process.platform === 'win32') {
    ...
    if (fs.existsSync(pngFile)) {
      const img = nativeImage.createFromPath(pngFile);
  ...
  if (process.platform === 'darwin') {
    ...
    if (fs.existsSync(file)) {
      ...
      const tmp = path.join(os.tmpdir(), 'geminitherapy-tray.icns');
      try {
...
app.whenReady().then(async () => {
  ...
  tray.setToolTip('Gemini Therapy – click for magic sparkle');
  tray.setContextMenu(
